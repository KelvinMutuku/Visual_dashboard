...
df = pd.DataFrame({
    ...
    'Product': np.random.choice(['Rice', 'Sugar', 'Salt'], size=100)
})
...
col1.metric("Total Sales", f"KES{filtered_df['Sales'].sum():,}")
col2.metric("Average Sales", f"KES{filtered_df['Sales'].mean():.0f}")
